...
const FLECK_SUPER_DRAW_W    = 60 * SCALE * 0.9 * 0.9 * 0.9 * 0.9 * 0.9 * 0.82 * 1.06 * 1.10; // etwas größer
...
const CLOUD_BIGGER_TINT     = [54,56,66];    // dunkler/deutlicher
const CLOUD_BIGGER_ALPHA    = 0.28;
...
const ORIGINAL_SCHL_COUNT = BASE_SCHL_COUNT + EXTRA_SCHL_COUNT;
...
function isSoftSchliereNumber(n) {
  return !NORMAL_SCHLIEREN_NUMBERS.has(n);
}
...
function schliereThicknessMul(sc) {
  if (sc._extraSoftSchliere) return 0.78;
  if (sc._softSchliere) return 0.88;
  return 1.0;
}
...
function resetSchliereRenderFlags(sc, n) {
  ...
  sc._softCenterExtra = false;
  sc._f2StretchSchliere = false;
  sc._softSchliere = isSoftSchliereNumber(n);
  sc._extraSoftSchliere = n > ORIGINAL_SCHL_COUNT;
  sc._fleckSchliere = FLECK_SCHLIEREN_NUMBERS.has(n);
  sc._pairedFleckSchliere = false;
  sc._extraPairedSchliere = false;
  sc._stableFogRender = true;
...
function drawSemicolonStyleReplica(label, x, y, r, aspect = 1.0, angleDeg = 0) {
  ...
  cells.forEach((c, i) => nucleusCell(c[0], c[1], c[2], c[3], i < 3, c[4]));
...
function drawRoundStyleSemReplica(label, x, y, r, aspect = 1.0, angleDeg = 0) {
  ...
  cells.forEach((c, i) => {
    ...
    const a = c[4];
    ctx.filter = 'blur(5px)';
    ctx.fillStyle = 'rgba(252,255,255,0.24)';
    ctx.beginPath();
    ctx.ellipse(px, py, rw * 1.08, rh * 1.08, a, 0, Math.PI * 2);
    ctx.fill();

    ctx.filter = 'blur(0.9px)';
    ctx.strokeStyle = 'rgba(70,78,90,0.36)';
    ctx.lineWidth = Math.max(0.32, r * 0.010);
    ctx.beginPath();
    ctx.ellipse(px, py, rw, rh, a, 0, Math.PI * 2);
    ctx.stroke();

    ctx.strokeStyle = 'rgba(250,254,255,0.62)';
    ctx.lineWidth = Math.max(0.22, r * 0.0045);
    ctx.beginPath();
    ctx.ellipse(px, py, rw * 1.04, rh * 1.04, a, 0, Math.PI * 2);
    ctx.stroke();

    ctx.strokeStyle = 'rgba(252,255,255,0.68)';
    ...
    ctx.fillStyle = i < 3 ? 'rgba(58,64,74,0.24)' : 'rgba(70,76,86,0.17)';
    ctx.beginPath();
...
function drawRoundTestStyleSemReplica(label, x, y, r, aspect = 1.0, angleDeg = 0, strength = 1) {
  const ctx = drawingContext;
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate(radians(angleDeg));
  ...
  cells.forEach(c => {
    ...
    ctx.filter = 'blur(5px)';
    ctx.fillStyle = `rgba(245,252,255,${0.09 * strength})`;
    ctx.beginPath();
    ctx.ellipse(px, py, rw * 0.92, rh * 0.92, a, 0, Math.PI * 2);
    ctx.fill();

    ctx.filter = 'blur(0.8px)';
    ctx.strokeStyle = `rgba(75,82,90,${0.42 * Math.min(1.18, strength)})`;
    ctx.lineWidth = Math.max(0.35, r * 0.007);
    ctx.beginPath();
    ctx.ellipse(px, py, rw, rh, a, 0, Math.PI * 2);
    ctx.stroke();

    ctx.strokeStyle = `rgba(245,252,255,${0.34 * strength})`;
    ctx.lineWidth = Math.max(0.25, r * 0.005);
    ctx.beginPath();
    ctx.ellipse(px, py, rw * 0.94, rh * 0.94, a, 0, Math.PI * 2);
    ctx.stroke();

    ctx.filter = 'blur(7px)';
    ctx.fillStyle = `rgba(55,60,68,${0.30 * Math.min(1.12, strength)})`;
    ctx.beginPath();
    ctx.ellipse(px, py, rw * 0.18, rh * 0.18, a, 0, Math.PI * 2);
    ctx.fill();
  });
...
function drawForm23RoundTestSemReplica(label, x, y, r, aspect = 1.0, angleDeg = 0, strength = 1.25) {
  const ctx = drawingContext;
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate(radians(angleDeg));
  ...
  function loop(cx, cy, rx, ry, rot, inner = false) {
    const px = cx * r;
    const py = cy * r;
    const rw = rx * r;
    const rh = ry * r;
    ...
    if (!inner) {
      ctx.filter = 'blur(5px)';
      ctx.fillStyle = `rgba(245,252,255,${0.08 * strength})`;
      ctx.beginPath();
      ctx.ellipse(px, py, rw * 0.96, rh * 0.96, rot, 0, Math.PI * 2);
      ctx.fill();
    }

    ctx.filter = 'blur(0.85px)';
    ctx.strokeStyle = `rgba(75,82,90,${0.42 * Math.min(1.18, strength)})`;
    ctx.lineWidth = Math.max(0.35, r * (inner ? 0.005 : 0.007));
    ctx.beginPath();
    ctx.ellipse(px, py, rw, rh, rot, 0, Math.PI * 2);
    ctx.stroke();

    ctx.strokeStyle = `rgba(245,252,255,${0.30 * strength})`;
    ctx.lineWidth = Math.max(0.22, r * 0.0045);
    ctx.beginPath();
    ctx.ellipse(px, py, rw * 0.92, rh * 0.92, rot, 0, Math.PI * 2);
    ctx.stroke();

    if (!inner) {
      ctx.filter = 'blur(7px)';
      ctx.fillStyle = `rgba(55,60,68,${0.22 * Math.min(1.10, strength)})`;
      ctx.beginPath();
      ctx.ellipse(px + rw * 0.08, py + rh * 0.03, rw * 0.14, rh * 0.14, rot, 0, Math.PI * 2);
      ctx.fill();
    }
  }

  function softStroke(points, widthMul = 0.006, alphaMul = 1) {
    const layers = [
      { blur: 5, width: widthMul * 2.8, alpha: 0.075 },
      { blur: 1.1, width: widthMul, alpha: 0.36 },
      { blur: 0.5, width: widthMul * 0.55, alpha: 0.22, light: true }
    ];
    layers.forEach(layer => {
      ctx.filter = `blur(${layer.blur}px)`;
      ctx.strokeStyle = layer.light
        ? `rgba(245,252,255,${layer.alpha * strength * alphaMul})`
        : `rgba(75,82,90,${layer.alpha * Math.min(1.18, strength) * alphaMul})`;
      ctx.lineWidth = Math.max(0.3, r * layer.width);
      ctx.beginPath();
      points.forEach((p, i) => {
        const px = p[0] * r;
        const py = p[1] * r;
        if (i === 0) ctx.moveTo(px, py);
          ...
          const cxp = (prev[0] + p[0]) * 0.5 * r;
          const cyp = (prev[1] + p[1]) * 0.5 * r;
  ...
  // Form 2: oberer Teil, zwei schräge Hohlkörper und ein kleineres Oval rechts unten.
  loop(-0.78, -1.28, 0.43, 0.22, 0.28);
  loop(-0.24, -1.02, 0.48, 0.24, 0.48);
  loop(0.78, -0.72, 0.33, 0.22, -0.92);
  softStroke([[-1.18, -1.44], [-0.86, -1.28], [-0.52, -1.16], [-0.14, -0.98], [0.10, -0.82]], 0.0065, 0.9);

  // Form 3: unterer Teil, großes liegendes Oval mit Innenkontur, schrägem Steg, kleinem Oval.
  loop(-0.42, 0.62, 0.84, 0.36, -0.36);
  loop(-0.42, 0.62, 0.66, 0.25, -0.36, true);
  softStroke([[0.24, 0.32], [0.52, 0.08], [0.78, -0.15], [0.98, -0.34]], 0.008, 1.0);
  loop(1.12, -0.50, 0.32, 0.25, 0.54);
  loop(1.12, -0.50, 0.22, 0.16, 0.54, true);

  ctx.filter = 'none';
  ctx.restore();
  if (label) drawSceneLabel(label, x + r * 0.88, y - r * 1.18);
}

function drawSchliere12StyleSemReplica(label, x, y, r, aspect = 1.0, angleDeg = 0) {
  const ctx = drawingContext;
  ctx.save();
  ctx.translate(x, y);
  ctx.rotate(radians(angleDeg));
  ...
  const paths = [
    [[-0.98, -1.22], [-0.96, -0.78], [-0.88, -0.31], [-0.78, 0.18], [-0.72, 0.58]],
    [[0.42, 1.04], [0.78, 1.28], [1.18, 1.42], [1.58, 1.44]]
  ];

  function strokePath(points, blurPx, widthMul, alpha, offsetY = 0) {
    ctx.filter = `blur(${blurPx}px)`;
    ctx.strokeStyle = `rgba(76,78,92,${alpha})`;
    ctx.lineWidth = Math.max(1, r * widthMul);
    ctx.beginPath();
    points.forEach((p, i) => {
      const px = p[0] * r;
      const py = p[1] * r + offsetY * r;
      if (i === 0) ctx.moveTo(px, py);
      else {
        const prev = points[i - 1];
        const cx = (prev[0] + p[0]) * 0.5 * r;
        const cy = (prev[1] + p[1]) * 0.5 * r + offsetY * r;
  ...
  paths.forEach(points => {
    strokePath(points, 9, 0.21, 0.095, 0.00);
    strokePath(points, 6.5, 0.15, 0.110, 0.022);
    strokePath(points, 3.6, 0.060, 0.155, -0.016);
    strokePath(points, 2.0, 0.028, 0.115, 0.00);
  });
...
class SchlierCloud {
  ...
  display() {
    ...
    if (this._extraSoftSchliere) {
      drawingContext.filter = `contrast(0.54) brightness(1.10) blur(${SCHLC_BLUR_PX + 14}px)`;
    } else if (this._softSchliere) {
      drawingContext.filter = `contrast(0.64) brightness(1.06) blur(${SCHLC_BLUR_PX + 9}px)`;
    } else if (this._midThicken) {
    ...
    if (SCHLIEREN_FOG_RENDER) {
      if (this._fleckSchliere && !this._stableFogRender) {
        if (this._pairedFleckSchliere) {
          ...
          drawingContext.filter = 'blur(7px)';
          noFill();
          stroke(88, 90, 104, 30 * pairAlphaMul);
          strokeWeight(max(3, dia * 0.38));
          beginShape();
          for (let i = 0; i <= 34; i++) {
            const t = i / 34;
            const x = drawLen * 0.48 - pairLen * 0.50 + t * pairLen;
            const y = sin(t * PI * (1.05 + 0.45 * abs(sin(phase))) + this.wavePhase * 0.52 + phase) * dia * 0.92
              + sin(t * PI * (2.65 + 1.25 * abs(cos(phase))) - this.wavePhase * 0.39 + phase * 0.7) * dia * 0.42
              + sin(t * PI * 5.2 + this.wavePhase * 0.67 + phase * 1.3) * dia * 0.18;
            curveVertex(x, y);
          }
          endShape();

          drawingContext.filter = 'blur(2.6px)';
          stroke(146, 148, 160, 24 * pairAlphaMul);
          strokeWeight(max(0.7, dia * 0.09));
          beginShape();
          for (let i = 0; i <= 34; i++) {
            const t = i / 34;
          ...
          drawingContext.filter = 'blur(8px)';
          noStroke();
          fill(86, 88, 102, 28 * pairAlphaMul);
          ellipse(blobX, blobY, dia * 2.2, dia * 1.3);
          drawingContext.filter = 'blur(3.2px)';
          fill(136, 138, 150, 22 * pairAlphaMul);
          ellipse(blobX, blobY, dia * 1.55, dia * 0.86);
          drawingContext.filter = 'none';
          drawingContext.restore();
          pop();
        ...
        const bodyLen = max(dia * 4.2, drawLen * (0.26 + 0.18 * abs(sin(this._shapeSeed * 0.53))));
        const startX = drawLen * 0.50 - bodyLen * 0.50;
        const phase = this._shapeSeed || 0;
        const bodyY = (t) => {
          return (
            sin(t * PI * (0.92 + 0.62 * abs(sin(phase))) + this.wavePhase * 0.58 + phase * 0.17) * 0.92 +
            sin((t - 0.20) * PI * (2.35 + 1.10 * abs(cos(phase))) - this.wavePhase * 0.44 + phase * 0.31) * 0.44 +
            sin(t * PI * (4.6 + 1.80 * abs(sin(phase * 0.7))) + this.wavePhase * 0.73) * 0.22
          ) * curveMag * 0.82;
        };
        drawingContext.filter = 'blur(8px)';
        noFill();
        stroke(82, 84, 98, 34);
        strokeWeight(max(3.2, dia * 0.64));
        beginShape();
        for (let k = 0; k <= 28; k++) {
          const tt = k / 28;
          curveVertex(startX + tt * bodyLen, bodyY(tt));
        }
        endShape();
        drawingContext.filter = 'blur(3px)';
        stroke(132, 134, 148, 26);
        strokeWeight(max(0.8, dia * 0.18));
        beginShape();
        for (let k = 0; k <= 28; k++) {
          const tt = k / 28;
          curveVertex(startX + tt * bodyLen, bodyY(tt));
      ...
      const fogPhase = this._stableFogRender ? this._shapeSeed * 0.37 : this.wavePhase;
      beginShape();
      for (let j = 0; j <= 36; j++) {
        const t = j / 36;
        const wave =
          sin(t * PI * (0.72 + 0.24 * abs(sin(this._shapeSeed))) + fogPhase * 0.34 + this._shapeSeed * 0.17) * 0.66 +
          sin((t - 0.18) * PI * (2.05 + 0.72 * abs(cos(this._shapeSeed))) - fogPhase * 0.23 + this._shapeSeed * 0.31) * 0.34 +
          sin(t * PI * (3.7 + 1.35 * abs(sin(this._shapeSeed * 0.7))) + fogPhase * 0.41) * 0.18;
        curveVertex(t * drawLen, wave * curveMag);
      }
      endShape();
      drawingContext.filter = 'blur(2.4px)';
      stroke(140, 142, 154, 22);
    ...
    noFill();
    if (this._f2StretchSchliere) stroke(54, 56, 70, 86);
    else if (this._centerDarkComplex) stroke(54, 56, 72, 150);
    else if (this._centerLongWave) stroke(72, 74, 88, 120);
    else if (this._midThicken) stroke(62, 64, 78, 124);
    else if (this._darker) stroke(66, 68, 82, 118);
    else stroke(92, 94, 108, 92);
...
const DARK_FLECK_LEFT_ALPHA     = 0.46;  // dunkler
...
class FloaterPath {
  ...
  drawFleckLocal(lx, ly) {
    ...
    drawingContext.ellipse(
      ...
      FLECK_SUPER_DRAW_H * 0.28,
      FLECK_SUPER_DRAW_W * 0.26,
      FLECK_SUPER_DRAW_H * 0.20,
      0.52,
      0,
      Math.PI * 2
    );
...
function draw() {
  ...
  const elementSpeedScale = mouseMoving ? targetSpeedScale : 0;
  oppositeMovement.mult(elementSpeedScale * directionChangeBoost);
  if (!schliereOppositeMovementDelayed) schliereOppositeMovementDelayed = createVector(0, 0);
  schliereOppositeMovementDelayed.lerp(oppositeMovement, mouseMoving ? 0.024 : 0.034);
  const oppositeMovementSchlieren = schliereOppositeMovementDelayed.copy();
  ...
  const cmpMotion = compareMotionOffset();
  {
    ...
    drawSceneLabel('Sem 23', rsemForm23.x + semR * 0.72, rsemForm23.y - semR * 1.15);
  }
